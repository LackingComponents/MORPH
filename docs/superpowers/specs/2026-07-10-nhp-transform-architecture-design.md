...
**Pick-in-NHP, persist-in-source — the wizard interaction rule (applies to every persistent anatomical point: condylar axis, the interincisal midline, and the *entire* cephalometric set).** The surgeon places each point while looking at the **NHP-posed** scene, but what is *stored* is its **CT/source coordinate** — the world hit un-posed at the moment of save via `× NhpShared⁻¹`. Changing NHP then renders the point at `NhpShared ∘ pointSource`, so it follows the anatomy; **no re-bake on commit**. The wizards already enforce this where picks are concerned: the split routes its picks through `inverseNhpMatrix` → source (`CondyleSplitWindow.xaml.cs:497-515`). The cephalometry overlay must follow the same rule. Its 2D-DRR-driven path (`Project2DTo3D`, `CephalometryOverlay.xaml.cs:872-878`) is **already source** — the DRR is generated from the raw volume with no NHP (`DrrGenerator.GenerateLateral/GeneratePA`), so a 2D pick projects back to a source coordinate. Its **3D-viewport-hit path is not**: `:809` stores a world/posed hit as `Position3D`; under the lazy model that must be un-posed to source on save, and the 3D markers (`Refresh3DLandmarks:914-952`, sphere built at `Position3D` with no `Transform`) must render posed — set the sphere's `Transform = NhpSharedTransform` and re-invoke `Refresh3DLandmarks` on NHP change. The `CommitNhp` ceph bake (`NhpViewModel.cs:141-156`) is deleted (§5.2); nothing re-bakes a ceph point, so the source-coordinate pick + posed render are the two halves that make the principle hold.
...
|---|---|---|---|
| **B1** | Split wizard returns (`Cranium/MandibleVertices`) and **all landmark/ceph picks** | **No bake.** Output in **source space** (the split already operates in `x*spacing` DICOM = source); `LocalTransform = Identity`. Every persistent anatomical point a wizard collects — the condylar axis, the interincisal midline, every cephalometric point — is picked in the NHP-posed view but **stored as its source coordinate** (the world hit `× NhpShared⁻¹`, just as the split already routes its picks through `inverseNhpMatrix` → `CondyleSplitWindow.xaml.cs:497-515`). | **P1** (DICOM-vs-NHP mismatch) dies for free — there is no forward-bake to forget. Points follow the anatomy under any later NHP because they render as `NhpShared ∘ pointSource`. |
...
| **INV4** | Landmarks live in **source space**, unchanged across NHP — including the **entire cephalometric set**; effective rendered/pivot position = `NhpShared ∘ landmark_source`. Picked in the NHP view, **stored** as the un-posed source coordinate. | A bake of a landmark on commit/profile switch; a 3D-viewport ceph pick stored as the posed world coord instead of source (`CephalometryOverlay.xaml.cs:809`); no re-pose on render. | Assert tuples unchanged across NHP ops/profile switch; assert surgical pivot = `NhpShared ∘ pivot_source`; place a ceph point on the 3D view, change NHP, assert it follows the anatomy and `Position3D` is unchanged. |
