...
General rule: **no wizard writes vertices.** Legitimate bakes are a short, closed list — anything outside it touching vertices is a bug.

| # | Bake point | Rule | Bug it kills |
|---|---|---|---|
| **B1** | Split wizard returns (`Cranium/MandibleVertices`) | Output in **source space**; `LocalTransform = Identity`. | **P1** (DICOM-vs-NHP mismatch) dies for free — no forward-bake needed. |
| **B2** | Cast registration at import (`ImportStlAsync` → `IcpAligner.Align`) | One-time bake of the ICP rigid into cast vertices → source space; `LocalTransform = Identity` after. Registration, not a pose. | Cast-from-own-frame vs CT-source consistency. |
| **B3** | Pieces spawned from an existing piece's posed geometry (splint, mirrored models) | Generate offsets in **posed space** (`NhpShared ∘ jaw.LocalTransform`), then map back to source via `inverse(NhpShared ∘ jaw.LocalTransform)`; store source vertices + `LocalTransform = jaw.LocalTransform`. | **Splint-order mismatch (audit MED-HIGH)** — splint seats under any NHP, any profile. |
...
`BakeTransformIntoVertices` stays; only these four call sites use it. `CondyleBox` is read at solve-time from *source-space* landmark + half-extents, then posed by `NhpShared` inside `SplintEngine` — its internal local-axis convention is its own; it's fed source-space inputs now.

**Gone with the bake:** the `CommitNhp` vertex/landmark/`VolumePivot` bake; the ledger's cumulative-bake on add; the **occlusion double-bake on reopen (audit HIGH)** — nothing bakes on load; **P4** (half-extents not NHP-baked) — half-extents live in source space and `NhpShared` maps the whole box at recompute, no per-commit re-bake to forget.
